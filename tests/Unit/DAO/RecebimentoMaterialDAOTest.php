...
class RecebimentoMaterialDAOTest extends TestCase {
    ...
    public function testRegistrarRecebimentoMaterial() {
        ...
        $item3 = new ItemRecebimento($recebimento, $itemRequisicao1, 10);
        $item4 = new ItemRecebimento($recebimento, $itemRequisicao2, 5);


        $recebimento->adicionarItem($item1);
        $recebimento->adicionarItem($item2);
        $recebimento->adicionarItem($item3);
        $recebimento->adicionarItem($item4);



        $this->assertNotNull($recebimentoDAO->salvar($recebimento));
        $this->assertEquals(40.00, $material1->getQuantidadeEstoque());
        $this->assertEquals(40.00, $material2->getQuantidadeEstoque());
        
        
        $this->assertEquals(StatusRequisicaoMaterial::CONCLUIDA_TOTAL, $requisicao->getStatus());
    }
